...
import { getTime } from '../utils/waypoints.js';
...
const renderOffers = (allOffers, checkedOffers, isDisabled) => allOffers.map((offer) => (
  `<div class="event__offer-selector">
    ...
    name="event-offer-luggage" ${checkedOffers.includes(offer.id) ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}>
...
const renderEditingPointDateTemplate = (dateFrom, dateTo, isDisabled) => (
  `<div class="event__field-group  event__field-group--time">
    ...
    name="event-start-time" value="${getTime(dateFrom)}" ${isDisabled ? 'disabled' : ''}>
    &mdash;
    <label class="visually-hidden" for="event-end-time-1">To</label>
    <input class="event__input  event__input--time" id="event-end-time-1" type="text"
    name="event-end-time" value="${getTime(dateTo)}" ${isDisabled ? 'disabled' : ''}>
  </div>`
);

const renderEditingPointTypeTemplate = (currentType, isDisabled) => Object.values(PointType).map((type) => (
...
const createEditingPointTemplate = (waypoint, destinations, allOffers, isNewPoint) => {
  const {price, type, destination, startDate, endDate, offers, isDisabled, isSaving, isDeleting} = waypoint;
...
export default class WaypointView extends AbstractStatefulView {
  ...
  reset = (waypoint) => {
    this.updateElement(
      WaypointView.parsePointToState(waypoint),
    );
  };
  ...
  #pointDestinationChangeHandler = (evt) => {
    evt.preventDefault();
    const destination = this._state.destination.find((dest) => dest.name === evt.target.value);
    this.updateElement({
  ...
  #formSubmitHandler = (evt) => {
    evt.preventDefault();
    this._callback.formSubmit(WaypointView.parseStateToPoint(this._state));
  };
  ...
  #formResetClickHandler = (evt) => {
    evt.preventDefault();
    this._callback.resetClick(WaypointView.parseStateToPoint(this._state));
  };
